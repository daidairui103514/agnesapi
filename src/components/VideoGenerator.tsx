  ...
  return (
    <div className="flex flex-col gap-8">
      <div className="flex justify-end gap-3 relative z-10 -mb-5 pr-2">
         <div className="flex items-center bg-white dark:bg-[#1c1c1e] border border-[rgba(0,0,0,0.05)] dark:border-[rgba(255,255,255,0.05)] rounded-full px-2 py-1 shadow-sm h-[32px]">
           <input
             type="text"
             value={queryTaskId}
             ...
             disabled={!apiKey}
             className="text-[12px] bg-transparent outline-none w-[150px] px-2 text-[#1d1d1f] dark:text-[#f5f5f7] placeholder:text-[#86868b] dark:text-[#a1a1a6] disabled:opacity-50"
           />
           <button
             onClick={() => {
             ...
             className="text-[12px] text-[#8c52ff] font-medium px-2 border-l border-[rgba(0,0,0,0.05)] dark:border-[rgba(255,255,255,0.05)] hover:opacity-80 transition-opacity disabled:opacity-50"
           >
             查询
           </button>
         </div>
         <button
            ...
            className={cn(
              ...
              showHistory ? "bg-[#8c52ff]/10 text-[#8c52ff] border-[#8c52ff]/20" : "bg-white dark:bg-[#1c1c1e] text-[#1d1d1f] dark:text-[#f5f5f7] hover:bg-[#f5f5f7] dark:hover:bg-[#2c2c2e] border-[rgba(0,0,0,0.05)] dark:border-[rgba(255,255,255,0.05)]"
            )}
         >
           <History size={14} />
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 h-full items-start">
                ...
                {history.length === 0 ? (
                  ...
                  history.map((item) => (
                    <motion.div 
                       ...
                       className={cn("group relative rounded-[16px] border border-[rgba(0,0,0,0.05)] dark:border-[rgba(255,255,255,0.05)] overflow-hidden transition-all flex hover:cursor-pointer",
                          previewItem?.url === item.url ? "bg-white dark:bg-[#1c1c1e] shadow-sm ring-2 ring-[#8c52ff]/30" : "bg-[#f5f5f7] dark:bg-[#000000] hover:bg-white dark:hover:bg-[#1c1c1e] hover:shadow-sm"
                       )}
                       onClick={() => {
                         setPreviewItem({url: item.url, prompt: item.prompt, taskId: item.taskId});
                       }}
                    >
                      <video src={item.url} className="w-32 h-24 object-cover shrink-0 bg-[#e8e8ed] dark:bg-[#2c2c2e]" muted />
                      <div className="p-3 flex-1 min-w-0 flex flex-col pointer-events-none">
                         ...
                         <p className="text-[13px] text-[#1d1d1f] dark:text-[#f5f5f7] line-clamp-3 flex-1" title={item.prompt}>{item.prompt}</p>
                         <div className="flex justify-between items-end mt-2">
                           ...
                           <div className="flex gap-1 pointer-events-auto">
                              <a 
                                ...
                                className="opacity-0 group-hover:opacity-100 text-[#86868b] dark:text-[#a1a1a6] hover:text-[#8c52ff] hover:bg-[#8c52ff]/10 p-1.5 rounded-full transition-all"
                              >
                                 <Play size={14} />
                              </a>
                              <button
        ...
        <motion.div 
          initial={{ opacity: 0, x: -20 }}
          animate={{ opacity: 1, x: 0 }}
          className="bg-white dark:bg-[#1c1c1e] p-5 sm:p-6 rounded-[24px] border border-[rgba(0,0,0,0.05)] dark:border-[rgba(255,255,255,0.05)] shadow-[0_8px_30px_rgba(0,0,0,0.04)] space-y-4 flex flex-col h-[600px] lg:h-[700px] overflow-y-auto custom-scrollbar"
        >
          <div>
            <label className="block text-[13px] font-semibold text-[#1d1d1f] dark:text-[#f5f5f7] mb-2">画面描述</label>
            <textarea
              value={prompt}
              onChange={(e) => setPrompt(e.target.value)}
              disabled={status === 'loading'}
              placeholder="描述您想要生成的视频场景细节，例如：运镜方向、主体动作、光影质感..."
              className="w-full p-3 bg-[#f5f5f7] dark:bg-[#000000] border border-transparent rounded-[16px] focus:outline-none focus:ring-2 focus:ring-[#8c52ff]/30 focus:border-transparent min-h-[100px] resize-y disabled:opacity-50 text-[15px] text-[#1d1d1f] dark:text-[#f5f5f7] placeholder-[#86868b] transition-all"
            />
          </div>

          <div>
             <div className="flex justify-between items-end mb-1.5">
              <label className="block text-[13px] font-semibold text-[#1d1d1f] dark:text-[#f5f5f7]">图像转视频源图 (选填)</label>
              <div className="relative">
                <select 
                   value={mode}
                   onChange={(e) => setMode(e.target.value)}
                   disabled={status === 'loading'}
                   className="appearance-none text-[12px] pl-2 pr-7 py-1 border border-[rgba(0,0,0,0.1)] dark:border-[rgba(255,255,255,0.1)] rounded-[8px] focus:outline-none focus:ring-2 focus:ring-[#8c52ff]/30 bg-[#f5f5f7] dark:bg-[#000000] text-[#1d1d1f] dark:text-[#f5f5f7] font-medium"
                >
                  <option value="standard">常规 / 图生视频 / 多图转换</option>
                  <option value="keyframes">关键帧动画</option>
                </select>
                <div className="absolute right-2 top-1.5 pointer-events-none text-[#86868b] dark:text-[#a1a1a6]">
                  <ChevronDown size={14} />
                </div>
              </div>
             </div>
            <textarea
              ...
              className="w-full p-3 bg-[#f5f5f7] dark:bg-[#000000] border border-transparent rounded-[12px] focus:outline-none focus:ring-2 focus:ring-[#8c52ff]/30 focus:border-transparent disabled:opacity-50 text-[14px] text-[#1d1d1f] dark:text-[#f5f5f7] placeholder-[#86868b] transition-all"
            />
          </div>

          <div className="grid grid-cols-2 gap-4">
            <div className="col-span-2">
               <label className="block text-[13px] font-semibold text-[#1d1d1f] dark:text-[#f5f5f7] mb-2">常用比例</label>
            ...
            <div className="relative">
              <label className="block text-[13px] font-semibold text-[#1d1d1f] dark:text-[#f5f5f7] mb-2">帧数</label>
              <select
                value={numFrames}
                onChange={(e) => setNumFrames(Number(e.target.value))}
                disabled={status === 'loading'}
                className="w-full pl-3 pr-8 py-2.5 appearance-none bg-[#f5f5f7] dark:bg-[#000000] border border-transparent rounded-[12px] focus:outline-none focus:ring-2 focus:ring-[#8c52ff]/30 disabled:opacity-50 text-[14px] text-[#1d1d1f] dark:text-[#f5f5f7] transition-all"
              >
              ...
              <div className="absolute right-2 top-[34px] pointer-events-none text-[#86868b] dark:text-[#a1a1a6]">
                <ChevronDown size={16} />
              </div>
            </div>
            <div>
              <label className="block text-[13px] font-semibold text-[#1d1d1f] dark:text-[#f5f5f7] mb-2">帧率 (FPS)</label>
              <input
                type="number"
                min="1"
                max="60"
                ...
                disabled={status === 'loading'}
                className="w-full px-3 py-2.5 bg-[#f5f5f7] dark:bg-[#000000] border border-transparent rounded-[12px] focus:outline-none focus:ring-2 focus:ring-[#8c52ff]/30 disabled:opacity-50 text-[14px] text-[#1d1d1f] dark:text-[#f5f5f7] transition-all"
              />
            </div>
          </div>
          ...
          <div className="mt-auto space-y-4">
            <div className="grid grid-cols-2 gap-4">
              <div>
                <label className="block text-[13px] font-semibold text-[#1d1d1f] dark:text-[#f5f5f7] mb-2">视频宽度 <span className="font-normal text-[11px] text-[#86868b] dark:text-[#a1a1a6] ml-1">(32/64的倍数)</span></label>
                <input
                  type="number"
                  step="64"
                  min="256"
                  ...
                  disabled={status === 'loading'}
                  className="w-full px-3 py-2.5 bg-[#f5f5f7] dark:bg-[#000000] border border-[rgba(0,0,0,0.05)] dark:border-[rgba(255,255,255,0.05)] rounded-[12px] focus:outline-none focus:ring-2 focus:ring-[#8c52ff]/30 disabled:opacity-50 text-[14px] text-[#1d1d1f] dark:text-[#f5f5f7] transition-all"
                />
              </div>
              <div>
                <label className="block text-[13px] font-semibold text-[#1d1d1f] dark:text-[#f5f5f7] mb-2">视频高度 <span className="font-normal text-[11px] text-[#86868b] dark:text-[#a1a1a6] ml-1">(32/64的倍数)</span></label>
                <input
                  type="number"
                  step="64"
                  min="256"
            ...
            <div className="bg-[#f5f5f7] dark:bg-[#000000] p-3 rounded-[12px] border border-[rgba(0,0,0,0.05)] dark:border-[rgba(255,255,255,0.05)] flex items-center gap-3 text-[13px] text-[#1d1d1f] dark:text-[#f5f5f7]">
              <Clock size={16} className="text-[#86868b] dark:text-[#a1a1a6]" />
        ...
        {/* Result & Logs Display */}
        <div className="bg-white dark:bg-[#1c1c1e] rounded-[24px] border border-[rgba(0,0,0,0.05)] dark:border-[rgba(255,255,255,0.05)] p-6 sm:p-8 flex flex-col h-[600px] lg:h-[700px] overflow-hidden relative shadow-[0_8px_30px_rgba(0,0,0,0.04)]">
          ...
          <div className="flex-1 flex flex-col items-center justify-center min-h-0 relative z-10 w-full h-full">
            ...
            {displayStatus === 'success' && displayVideo && (
              <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="w-full flex-col flex items-center justify-center mb-4 relative group">
                ...
                <div className="w-full aspect-video rounded-[16px] overflow-hidden bg-[#e8e8ed] dark:bg-[#2c2c2e] flex items-center justify-center border border-[rgba(0,0,0,0.05)] dark:border-[rgba(255,255,255,0.05)] shadow-[0_8px_32px_rgba(0,0,0,0.08)] relative">
                  <video 
                    src={displayVideo} 
                    controls
                    autoPlay
                ...
                {displayPrompt && (
                  <div className="mt-6 px-4 py-3 bg-[#f5f5f7] dark:bg-[#000000] rounded-[12px] w-full text-[13px] text-[#1d1d1f] dark:text-[#f5f5f7] shadow-sm max-h-[100px] overflow-y-auto break-words custom-scrollbar">
                    {displayPrompt}
                  </div>
                )}
              </motion.div>
          ...
          <AnimatePresence>
          {showLogs && (
            <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: 160, opacity: 1 }} className="shrink-0 bg-[#f5f5f7] dark:bg-[#000000] rounded-[16px] border border-transparent p-5 font-mono text-[12px] text-[#86868b] dark:text-[#a1a1a6] overflow-y-auto mt-4 custom-scrollbar">
              {pollLog.map((log, i) => (

...
import { Settings, MessageSquare, Image as ImageIcon, Video as VideoIcon, X, User as UserIcon, Bot, Sparkles, CheckCircle2, Lock } from 'lucide-react';
  ...
  if (!config.loaded) {
    return (
      <div className="min-h-screen bg-[#f5f5f7] flex items-center justify-center">
        <div className="flex flex-col items-center gap-4 text-[#86868b] animate-pulse">
  ...
  if (config.requirePassword) {
    return (
      <div className="min-h-screen bg-[#f5f5f7] flex items-center justify-center p-4">
        <div className="bg-white rounded-[24px] p-8 shadow-[0_16px_64px_rgba(0,0,0,0.08)] w-full max-w-sm animate-in fade-in zoom-in-95 duration-300">
          <div className="flex flex-col items-center mb-8">
            <div className="w-12 h-12 bg-[#0071e3]/10 text-[#0071e3] rounded-full flex items-center justify-center mb-4">
              <Lock size={24} />
            </div>
            <h1 className="text-[20px] font-semibold text-[#1d1d1f] tracking-tight">需要访问密码</h1>
            <p className="text-[13px] text-[#86868b] mt-1 text-center">当前站点已设置密码保护</p>
          </div>
          ...
          <form onSubmit={handleLogin} className="space-y-4">
            <div>
              <input
                ...
                placeholder="请输入网站密码"
                className="w-full p-3 bg-[#f5f5f7] border border-transparent rounded-[12px] focus:outline-none focus:ring-2 focus:ring-[#0071e3]/50 focus:bg-white text-[14px] text-[#1d1d1f] transition-all text-center tracking-widest"
                autoFocus
              />
